...
- `dist/`: Output directory for bundled files (generated by build scripts)
...
# Start the demo server (http://localhost:9000)
npm run demo
```
...
- The project uses `@vscode-elements/webview-playground` for VS Code theming preview
- Custom elements from the webview-playground are accessed via TypeScript declarations in `src/custom-elements.d.ts`
- Demo page in `index.html` includes the VSCode playground components with proper theme integration

## Autocomplete Implementation:
- Components handle filtering suggestions based on user input
...
- React components use ref forwarding pattern for imperativeHandle access
- Component props follow naming convention: `VSCode[ComponentName]Props`
- Strict typing with full TypeScript/React integration
